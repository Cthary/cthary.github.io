...
test("Calculator.rollDice", (t) => {
    ...
    const rolls = calculator.rollDice(1, 1, []);
    t.true(rolls[0] >= 1);
});
...
test("Simulator.parseSimulatedResultsByAmount (x10)", (t) => {
    ...
    const amount = 10;
    const result = simulator.simulateAmount(weapon, defender, amount);
    const results = simulator.parseSimulatedResultsByAmount(amount, result);
...
test("Simulator.parseModelsDestroyed", (t) => {
    ...
    const results = simulator.parseModelsDestroyed(damages, defender);
    t.true(results === 2);
});
...
test("Simulator.createResults", (t) => {
    ...
    const simulator = new Simulator(1);
    const amount = 1;
    const results = simulator.createResults(attacker, defenders, amount);
